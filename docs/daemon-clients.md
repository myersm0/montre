...
`DaemonClient::connect_or_spawn(corpus_path)` does all of this. Other-language clients implement the equivalent dance: BLAKE3-16-hex of the canonicalized corpus path → socket path under `~/.local/share/montre/sockets/` → connect → on `ENOENT` or `ECONNREFUSED`, unlink any stale socket, spawn `montre serve` detached, poll with backoff (10-second total timeout). See the protocol doc's "Discovery" and "Auto-spawn" sections for the full algorithm; `crates/montre-daemon/src/client.rs` is the reference implementation.
